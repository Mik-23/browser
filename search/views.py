...
def search_view(request):
    ...
    if response.status_code == 200:
        ...
        return render(request, 'search/result.html', {'results': results, 'query': query, 'page': page_number, 'total_results': len_results})
    else:
...
def image_view(request):
    ...
    offset = (page_number - 1) * 10
    url = f'https://yandex.ru/images-xml/?folderid={folderid}&apikey={api_key}&text=порно {encoded_query}&offset={offset}&itype=all'
    headers = {
    ...
    if response.status_code == 200:
        ...
        return render(request, 'search/images.html', {'images': images, 'query': query, 'page': page_number})
    else:
        print(f"Error: {response.status_code} - {response.text}")
        return HttpResponse("Error occurred", status=response.status_code)
...
def video_view(request):
    ...
    if response.status_code == 200:
        ...
        for item in soup.find_all('a'):  # Предположительно, ссылки представлены в теге <a>
            ...
            if ('yandex.ru' not in link and 'rutube.ru' not in link and ('http://noodlemagazine.com' in link or 'http://prostoporno.guru' in link
                or 'http://ukdevilz.com' in link or 'http://semyana.cc' in link
                or 'https://www.xvideos-ru3.com' in link)):
